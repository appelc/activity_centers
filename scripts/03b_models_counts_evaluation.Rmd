...
## format counts for females and males
  ...
  #keep necessary columns
  fmSTOC <- acData[,c('FEMALE_N','MALE_N','duration','Distance','dist_intended','reproState')]
  
  #convert NAs to 0s and order levels
  fmSTOC$count_female <- ifelse(is.na(fmSTOC$FEMALE_N), 0, fmSTOC$FEMALE_N)
  fmSTOC$count_male <- ifelse(is.na(fmSTOC$MALE_N), 0, fmSTOC$MALE_N)
