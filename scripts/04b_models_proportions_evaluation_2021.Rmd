---
title: "Activity center models - days with detections (evaluation) - 2021"
author: "Cara"
date: "`r Sys.Date()`"
output: 
  html_document:
...
###  
#### Comparing ways to deal with overdispersion (quasibinomial, random effect, glmmTMB, etc.)
#### **Decided to use quasi model with fixed effect and/or random effect for site moving forward**
...
mod3.null <- glmer(cbind(nights_STOC_ANY, duration - nights_STOC_ANY) 
                   ~ 1 + (1|SITESTN), family = binomial(link = 'logit'), data = daysMerge)
summary(mod3.null)
...
mod3.repro.dist.tmb <- glmmTMB(cbind(nights_STOC_ANY, duration - nights_STOC_ANY)
                              ~ reproState * Distance + (1|SITESTN), data = daysMerge, 
                              family = binomial(link = 'logit')) 
